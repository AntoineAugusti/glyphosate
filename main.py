...
def code_departement(departement):
    departement_nom = difflib.get_close_matches(
        departement, [d["nom"] for d in departements], 1
    )[0]
    return [d["code"] for d in departements if d["nom"] == departement_nom][0]


resp = requests.get("https://glyphosate.gouv.fr")
resp.raise_for_status()


matches = re.findall(r"data-tippy=\"((?:.|\n)*?)\"", resp.content.decode("utf-8"))
...
for match in matches:
    ...
    # """Mayotte :
    # <br />
    # Nombre d'agriculteurs engagés : 10"""
    splitted = match.split(":")
    if len(splitted) != 3:
        raise ValueError
    departement = splitted[0].strip()
    try:
        nombre_engages = int(splitted[-1].strip())
    except ValueError:
        continue
    res.append(
        {
